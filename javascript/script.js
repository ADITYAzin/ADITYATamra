...
function renderProjects() {
    const t = content[state.lang];
    const categories = state.lang === 'id' ? ['Semua', 'UI/UX Design', 'Brand Design', 'Graphic Design', 'Lainnya'] : ['All', 'UI/UX Design', 'Brand Design', 'Graphic Design', 'Others'];
    const filterMap = {
        'Semua': 'All', 'UI/UX Design': 'UI/UX Design', 'Brand Design': 'Brand Design', 'Graphic Design': 'Graphic Design', 'Lainnya': 'Lainnya',
        'All': 'All', 'UI/UX Design': 'UI/UX Design', 'Brand Design': 'Brand Design', 'Graphic Design': 'Graphic Design', 'Others': 'Others'
    };
    ...
    if (tabHeaders) {
        tabHeaders.innerHTML = availableTabs.map(tabKey => `
            <button onclick="setDetailTab('${tabKey}')" 
                ...
                ${tTabs[tabKey] || tabKey.charAt(0).toUpperCase() + tabKey.slice(1)}
            </button>
        `).join('');
    }
        ...
        <div class="max-w-6xl mx-auto px-6 py-20">
            <div class="flex flex-col md:flex-row gap-16 items-start">
                <div class="w-full md:w-1/3 space-y-8 animate-enter">
                     <div class=" rounded-3xl overflow-hidden relative shadow-card group"><img src="img/herosec.png" class="w-full h-full object-contain transition-transform duration-700 group-hover:scale-105" alt="Profile"></div>
                    <div class="grid grid-cols-2 gap-4"><div class="bg-white border border-slate-200 p-6 rounded-2xl text-center shadow-soft"><div class="text-3xl font-extrabold text-blue-400">14</div><div class="text-xs text-slate-400 mt-1 font-bold uppercase tracking-wide">${t.stats.projects}</div></div><div class="bg-white border border-slate-200 p-6 rounded-2xl text-center shadow-soft"><div class="text-3xl font-extrabold text-slate-900">10</div><div class="text-xs text-slate-500 mt-1 font-bold uppercase tracking-wide">${t.stats.awards}</div></div></div>
                </div>
                <div class="w-full md:w-2/3 animate-enter" style="animation-delay: 100ms">
                    ...
                    <div class="space-y-12">
                        <div><h3 class="font-bold text-slate-900 border-b border-slate-200 pb-3 mb-6 uppercase text-sm tracking-widest">${t.workStyle}</h3><div class="flex flex-wrap gap-4">${t.styles.map(s => `<div class="flex items-center gap-3 px-5 py-3 bg-white border border-slate-200 rounded-xl shadow-soft"><i data-lucide="check-circle" class="w-5 h-5 text-primary"></i> <span class="font-bold text-slate-700">${s}</span></div>`).join('')}</div></div>
                        <div class="bg-slate-50 p-8 rounded-3xl border border-slate-100">
                            <div class="mb-8"><h3 class="text-xl font-bold text-primary mb-6">${t.sectionHardSkills}</h3><div class="flex flex-wrap gap-2 mb-10">${t.hardSkills.map(s => `<span class="px-4 py-2 bg-white text-slate-700 text-sm font-bold rounded-lg shadow-sm border border-slate-200 hover:border-primary/50 hover:text-primary transition-colors cursor-default">${s}</span>`).join('')}</div></div>
                            <div class="mb-8"><h3 class="text-xl font-bold text-primary mb-6">${t.sectionTools}</h3><div class="flex flex-wrap gap-2 mb-10">${t.tools.map(s => `<span class="px-4 py-2 bg-white text-slate-700 text-sm font-bold rounded-lg shadow-sm border border-slate-200 hover:border-primary/50 hover:text-primary transition-colors cursor-default">${s}</span>`).join('')}</div></div>
                            <div class="pt-8 border-t border-slate-200"><h3 class="text-xl text-primary font-bold mb-6">${t.education}</h3>${t.eduList.map(e => `<div class="mb-6 last:mb-0 flex gap-4 items-start"><div class="w-12 h-12 rounded-2xl bg-blue-100 text-primary flex items-center justify-center shrink-0"><i data-lucide="graduation-cap" class="w-6 h-6"></i></div><div><div class="font-bold text-slate-900 text-lg">${e.title}</div><div class="text-slate-500 font-medium">${e.school}</div></div></div>`).join('')}</div>
                        </div>
                    </div>
                </div>
            </div>
